...
| 章节 | 内容 | 你会得到 |
|---|---|---|
| [00 心智模型](00-心智模型.md) | 先建立正确的心智模型 | 修正「沙箱 = 牢房」的误解；说清赋能与约束、沙箱与容器/VM 的区别 |
| [01 定位](01-定位.md) | 从「防御机制」到「Agent 的手脚」 | 可写进设计文档的定义；明确沙箱**不**负责什么 |
| [02 为什么需要沙箱](02-为什么需要沙箱.md) | Agent 的六个刚需 | 一个多步任务的逐步拆解；三个 Agent 独有变量 |
| [03 能力清单](03-能力清单.md) | 一个沙箱应提供的能力 | 最小接口集合、P0–P2 优先级、每项的坑与做法 |
| [04 架构](04-架构.md) | 控制面与数据面 | 职责拆解、完整时序、预热池与会话粘性、反模式清单 |
| [05 隔离后端选型](05-隔离后端选型.md) | Docker / gVisor / microVM / VM / WASM | 决策树、选择口诀、gVisor vs microVM、升级路径 |
| [06 威胁模型](06-威胁模型.md) | Prompt Injection 与数据外泄 | 两类注入、STRIDE、纵深防御链、白名单代理设计 |
| [07 产品版图](07-产品版图.md) | 托管与自建选项 | 评估产品的 8 个问题、选型三问、自建 vs 托管 |
| [08 何时不需要沙箱](08-何时不需要沙箱.md) | 避免过度设计 | 决策树、L0–L3 分级、过度设计的代价 |
| [09 实战搭建](09-实战搭建.md) | bwrap 沙箱 → Docker → MCP 工具 | 可运行步骤、参数对照、防目录穿越的 MCP 封装 |
| [10 趋势与延伸](10-趋势与延伸.md) | 沙箱要往哪走 | 五个趋势、八条延伸轴、按角色的下一步 |
| [附录 A 速记](附录A-速记.md) | 核心结论 | 分主题的一句话版本 + 30 秒决策流 |
| [附录 B 术语表](附录B-术语表.md) | 术语解释 | 按主题分组的 40+ 术语，含「为什么重要」 |
| [附录 C 安全检查清单](附录C-安全检查清单.md) | 上线前逐条核对 | 带红线的清单 + 最小可执行配置 |
