...
[← 上一章：附录 B](附录B-术语表.md) | [返回目录](README.md)
...
> ⚠️ bwrap 本身**不设置** cgroup / rlimit 限额。上表的 CPU / 内存 / 磁盘 / 时长限制需配合 `ulimit`、外层 cgroup 或执行包装器的超时来实现；参考 [`../agent-sandbox-lab/exp/04-resource-limits.sh`](../agent-sandbox-lab/exp/04-resource-limits.sh)。
